...
;define(function(require, exports, module){
		...
		init : function(){
			G.ajax({
				...
				success : function( response ){
					...
					G.Global('modal', modal);
					G.Global('indexPanel', indexPanel); //记录主panel为全局变量
							...
							success: function(res){
								res.result == 1 ? (function(){
									...
									if( config.type === 'table' ){
										...
										G.Global('table', new Table( config.conf )); //表格模式需要记录表格当前主体对象到全局变量
	...
	function cacheLoginMessage(){ //缓存登录信息
		G.ajax({ url:'loginMessage',
			success: function(data){
				G.Global('loginMessage', data);
			}
		});
	}
});