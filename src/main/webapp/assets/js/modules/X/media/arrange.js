...
;define(function(require, exports, module){
	...
	var indexPanel = G.Global('indexPanel'), 
	...
	var authCode = G.getUrlParam('authCode');//UCGS用户id
	var auth = ",string:AuthCode@"+authCode;
	if(authCode == null){auth='';authCode=''}else{ auth = ",string:AuthCode@"+authCode;}
	
	var permission = '10086';//G.getUrlParam('pmssionCode'); //是否显示审核按钮 值 10086：显示按钮 10010:不显示按钮 
	...
	module.exports = {
		init: function( config ){
			...
			if(permission != 10086){ //显示审核按钮，并去掉authCode条件
				_image_.util.splice(3,1); //从按钮数组中删除审核按钮	
			}else{
				authCode = '';
				auth = '';
				_image_.uri = 'media/getImages?deleted=0';
			}
			...
			target.find('.spin-timer-shaft>.timer-content').on('mouseover', function(){
				...
				buffer && currPlace && (function(){
					...
					bufferMedias.get( buffer.ID ) ? G.toastr('warning', '占位符媒资重复') : (function(){
						var item = currPlace.type === 'Video' ? {type:'Video', ID:buffer.ID, posterUri: uams_resource_ip + buffer.PosterUrl, fullUri:uams_resource_ip + buffer.VideoUrl, title:buffer.Title, size:buffer.VideoSize, grade:buffer.Grade, duration:buffer.Duration}
						: {type:'Image', ID:buffer.ID, fullUri: uams_resource_ip + buffer.Url, title:buffer.Title, size:buffer.Size, width:buffer.Width, height:buffer.Height};
			...
			this.image( _image_ );
			jQuery("body").on('click','.__media_item__',function(){
				jQuery(this).find('input[type="checkbox"][name="imageItem"]').prop('checked',true);
			});
		},
		place : function( _place_ ){
			var ucms = _place_.ucms,
				guid = G.getUrlParam('guid'), //'931ad090-12b3-4700-8ad9-10f0a1a0fae5'
				//authCode = G.getUrlParam('authCode'),
		...
		video : function( _video_ ){
			...
			videoGroup = new Group({
				...
				uri: _video_.uri,
				pageSize: 12,
				sifter: _video_.sifter+auth || '',
				orderCase: 'CreateTime desc',
				itemClass: '__media_item__',
				util: _video_.util,
				extend: _video_.extend,
				context: function( item ){
					return  '<span class="item-image"><img src="' + uams_resource_ip + item.PosterUrl + '" height="90%"/></span>' +
		...
		image : function( _image_ ){
			...
			imageGroup = new Group({
				...
				pageSize: 40, 				
				orderCase: 'ID desc',
				sifter: _image_.sifter+ auth || '',
				itemClass: '__media_item__',
				util: _image_.util,
				extend: _image_.extend,
				context: function( item ){
					return  '<span class="item-image"><img src="' + item.Url + '" height="100%"/></span>' +
							'<span class="item-info"><i class="badge">' + (item.Title || '暂无')  + '</i></span>' + 
							'<span class="item-info">大小：' + sizeFormat( item.Size ) + '</span>\
							<input type="checkbox" value="'+ item.ID +'" name="imageItem"/>';
				},
				bind: function( items ){
					/*
					this.target.find('.item-image>img').error(function(){
...
//						console.log(items[G.$(this).index()]);
		...
		uploadImage: function( btn ){
			...
			new Unit.UploadImage({
				...
				uri: 'upload/uploadFile?authCode='+authCode,
				bind: function(){
					modal.hide();
		...
		selector : function(btn){ //按类型选择
			console.debug(btn);
			imageGroup.refresh(null,{uri:'media/getImages?deleted=' + btn.val});
		},
		check : function(btn){//审核操作
			//todo
			console.debug(btn);
			var cker = jQuery('input[type="checkbox"][name="imageItem"]:checked');
			if(cker.size() > 0){
				var imgIds = [];
				cker.each(function(){
					imgIds.push(this.value);
				});
				console.debug(imgIds);

				jQuery.ajax({
					url : 'media/checkImage',
					type : 'post',
					data : {'imgIds':imgIds,'Deleted':btn.val},
					dataType : 'json',
					success : function(d){
						if(d > 0){
							G.toastr('info', '操作成功');
							imageGroup.refresh(null,{uri:'media/getImages?deleted=0'});
						}
					},
					error : function(e){
						G.toastr('warning', '系统发生错误');
					}
				});
			}else{
				G.toastr('warning', '请先选择资源...');
			}
		}
	}
});
