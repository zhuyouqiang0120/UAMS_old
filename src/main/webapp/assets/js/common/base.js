...
;define(function(require, exports, module){
	...
	global.Extend(global, {
		...
		Global : function(){ //全局变量操作
        	var args = arguments;
        	window._stack_ = window._stack_ || new global.HashMap();
        	
        	if(args.length == 0) return false;
        	if(typeof args[0] !== 'string') return false;
        	
        	if(args.length === 1){
    			return window._stack_.get(args[0]);
        	}else{
        		window._stack_.put(args[0], args[1]);
        	}
        }
	}, false);
