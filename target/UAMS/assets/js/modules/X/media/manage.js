...
;define(function(require, exports, module){
	...
	var modal = _Static_.get('modal'),
		table = _Static_.get('table'), //全局变量
	...
	module.exports = {
		episode: function( _t_ ){
			...
			modal.title(_t_).show().bind(function(){
		...
		pack: function(_t_, _items_){//媒资单片打包到剧集
			modal.body('');
			var	media = null,
				group = new Group({
					target: modal.target.find('.modal-body'),
		...
		detail: function(_t_, _item_){//媒资详情
			modal.body('');
			if(_item_.Type === 2){ //剧集
				new Group({
					target: modal.target.find('.modal-body'),
			...
			modal.title(_t_).show().foot('hide');
		},
		separate: function(_t_, _item_){
			modal.body('');
