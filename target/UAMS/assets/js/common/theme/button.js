...
;define(function(require, exports, module){
	...
	function Button( _o_ ){
		...
		var modal = new Unit.Modal({target:button.target.parent().parent(), showFooter: true });
		...
		this.operate = function( button, field, selectItems, callback ){
			...
			switch( button.func ){
				...
				case 'extend': //对数据执行扩展方法
					...
					if( conf.item === 'none' ){
						extend[conf.entry]( buttonName );
						return false;
					}else if( conf.item === 'single' ){
						if( selectItems.length === 1 ){
							extend[conf.entry]( buttonName, selectItems[0] );
							return false;
						}else{
							warn = selecItems.length === 0 ? '请先选择您需要 ' + buttonName + ' 的信息~' : '请只选择一条信息进行操作~';
						}
					}else if( conf.item === 'mulit' ){
						if(selectItems.length === 0){
							warn = '请先选择您需要 ' + buttonName + ' 的信息~';
						}else{
							extend[conf.entry]( buttonName, selectItems );
							return false;
						}
					}
			}
